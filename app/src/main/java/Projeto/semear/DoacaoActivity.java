...
//                }
//
//                String cleanString = s.toString().replaceAll("[R$,.\\s]", "").replace(",", ".");
...
//        }
//
//        Toast.makeText(DoacaoActivity.this, "Dados OK, você será redirecionado para o Qrcode de pagamento", Toast.LENGTH_LONG).show();
//
...
//        intent.putExtra("valor", inputValor.getText().toString().trim());
//
...
//        }
//
//        // Verifica se contém letras maiúsculas - não permite
...
//        }
//
//        String valorLimpo = valorFormatado.replace("R$", "")
//                .replaceAll("[^\\d,\\.]", "")
//                .trim();
//
...
public class DoacaoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnEscolherValor.setOnClickListener(v -> {
            // Muda a cor do botão para laranja e o texto para branco
            v.setSelected(true);  // Marca o botão como "selecionado"
            ((Button) v).setTextColor(ContextCompat.getColor(DoacaoActivity.this, R.color.white));
            v.setBackgroundColor(ContextCompat.getColor(DoacaoActivity.this, R.color.orange)); // Cor laranja

            // Restante da ação de selecionar valor
            inputNome.requestFocus();
            InputMethodManager imm = (InputMethodManager) DoacaoActivity.this.getSystemService(Context.INPUT_METHOD_SERVICE);
            imm.showSoftInput(inputNome, InputMethodManager.SHOW_IMPLICIT);
        });
    ...
    // Novo método para validar e redirecionar para QrcodeActivity
    private void validarFormularioEIrParaQrcode() {
        if (!validarNome()) {
            inputNome.requestFocus();
            return;
        ...
        Toast.makeText(DoacaoActivity.this, "Dados enviados com sucesso!", Toast.LENGTH_LONG).show();
